...
function mockFeatures(): CafeFeatures {
  return {
    loyalty: {
      enabled: true,
      stampsPerReward: 10,
...
describe('applyLoyaltyAfterKdsComplete', () => {
  beforeEach(() => {
    ...
    applyLedgerStampAndRewards.mockResolvedValue({ cardProgress: 1, inserted: true });
  ...
  it('awards loyalty for a signed-in Stripe-confirmed app order', async () => {
  ...
  it('awards loyalty for pay-in-store unpaid orders', async () => {
    const { applyLoyaltyAfterKdsComplete } = await import('./loyalty-after-kds-complete.js');
  ...
  it('skips guest orders without a customer id', async () => {
    const { applyLoyaltyAfterKdsComplete } = await import('./loyalty-after-kds-complete.js');
  ...
  it('does not double-apply counters when the stamp ledger row already exists', async () => {
    applyLedgerStampAndRewards.mockResolvedValue({ cardProgress: 4, inserted: false });
