...
const CUSTOMER_ACTIVE_STATUSES = ['pending', 'confirmed', 'preparing', 'ready'] as const;
const CUSTOMER_CANCELLABLE_STATUSES = ['pending', 'confirmed', 'preparing', 'ready'] as const;
const CUSTOMER_TERMINAL_STATUSES = ['completed', 'cancelled'] as const;

async function normalisedOrdersFromRows(ordersRows: OrderRowDb[]): Promise<NormalisedOrder[]> {
...
export async function listCustomerOrdersForUser(params: {
  ...
  const [active, recent] = await Promise.all([
    normalisedOrdersFromRows(activeRes.rows),
    normalisedOrdersFromRows(recentRes.rows),
  ]);
