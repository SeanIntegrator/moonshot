...
export function redeemLoyaltyReward(rewardId: string): Promise<RedeemRewardResponse> {
  return apiFetch<RedeemRewardResponse>(
    `/loyalty/rewards/${encodeURIComponent(rewardId.trim())}/redeem`,
    { method: 'POST' },
  );
}
